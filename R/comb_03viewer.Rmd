...
packages <- c(
  ...
  "limma", "DT", "purrr", "dplyr", "ggplot2", "ggsankey"
)
...
renderPlot(
  {
    ...
    for (i in seq_along(input[[paste0("deg_conn", idx)]])) {
      if (
        ...
        # gene_ids <- gene_ids[match(gse_sub$GeneSet, names(gene_ids))]
    ...
    links_comb <- make_long(links_comb, source, target)
    links_comb$node <- factor(
      links_comb$node, levels = rev(unique(links_comb$node))
    )

    key <- unique(links_comb$node[links_comb$x == "target"])
    value <- hue_pal()(length(key))
    col_map <- setNames(value, key)
    links_comb$fill <- unfactor(links_comb$node)
    links_comb$fill <- ifelse(
      links_comb$x == "target", yes = col_map[links_comb$fill],
      no = links_comb$fill
    )
    for (i in seq(2, nrow(links_comb), by = 2)) {
      links_comb$fill[i - 1] <- links_comb$fill[i]
    }
    links_comb$node_fill <- ifelse(
      links_comb$x == "target", yes = links_comb$fill, no = "lightgray"
    )
