...
```{r, include = FALSE}
corr <- duplicateCorrelation(
  logcounts(deg), design = design_deg, block = deg$donor_id
)
fit <- lmFit(
  logcounts(deg), design_deg,
  correlation = corr$consensus.correlation, block = deg$donor_id
)
...
```{r, include = FALSE}
corr <- duplicateCorrelation(
  logcounts(gse), design = design_gse, block = gse$donor_id
)
fit <- lmFit(
  logcounts(gse), design_gse,
  correlation = corr$consensus.correlation, block = gse$donor_id
)
