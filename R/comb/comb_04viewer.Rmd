...
data_dir <- file.path("..", "..", "cache", "comb")
gene_sets <- readRDS(file.path(data_dir, "step02", "gene_sets.rds"))
gene_sets_keep <- readRDS(file.path(data_dir, "step02", "gene_sets_keep.rds"))
gene_anno <- readRDS(file.path(data_dir, "step02", "gene_anno.rds"))
deg_data <- readRDS(file.path(data_dir, "step03", "deg_data.rds"))
gse_data <- readRDS(file.path(data_dir, "step03", "gse_data.rds"))
deg_list <- readRDS(file.path(data_dir, "step03", "deg_list.rds"))
gse_list <- readRDS(file.path(data_dir, "step03", "gse_list.rds"))
gse_corrections <- readRDS(file.path(data_dir, "step03", "gse_corrections.rds"))
deg_corrections <- readRDS(file.path(data_dir, "step03", "deg_corrections.rds"))
...
top_anno <- HeatmapAnnotation(
  ...
  col = list(
    Donor = c(
      "Donor1" = anno_color[1], "Donor2" = anno_color[2],
      "Donor3" = anno_color[3], "Donor4" = anno_color[4],
      "Donor5" = anno_color[5], "Donor6" = anno_color[6],
      "Donor7" = anno_color[7], "Donor8" = anno_color[8]
    )
  ),
...
deg <- deg_data[[1]]
design_deg <- model.matrix(~ 0 + deg$tau)
...
gse <- gse_data[[1]]
design_gse <- model.matrix(~ 0 + gse$tau)
...
ctype_name <- vector("list", length = 4)
deg_corrected <- vector("list", length = 4)
gse_corrected <- vector("list", length = 4)
deg_range <- vector("list", length = 4)
gse_range <- vector("list", length = 4)
results_deg <- vector("list", length = 4)
results_gse <- vector("list", length = 4)
sig_deg <- vector("list", length = 4)
sig_gse <- vector("list", length = 4)
sig_uniq_deg <- vector("list", length = 4)
sig_uniq_gse <- vector("list", length = 4)
```
...
```{r, include = FALSE}
corr <- duplicateCorrelation(logcounts(deg), design_deg, block = deg$donor)
fit <- lmFit(
  logcounts(deg), design_deg,
...
deg_corrected[[idx]] <- removeBatchEffect(
  logcounts(deg), batch = deg$donor, group = deg$tau
)
...
results_deg[[idx]][ , col] <- deg_corrections[length]
...
```{r, include = FALSE}
corr <- duplicateCorrelation(logcounts(gse), design_gse, block = gse$donor)
fit <- lmFit(
  logcounts(gse), design_gse,
...
gse_corrected[[idx]] <- removeBatchEffect(
  logcounts(gse), batch = gse$donor, group = gse$tau
)
...
length <- length1:length2
results_gse[[idx]][ , col] <- gse_corrections[length]
