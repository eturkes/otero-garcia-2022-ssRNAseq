...
# -------------------------------------------------------------------------
metadata_to_plot <- vector("list", length = 10)
names(metadata_to_plot) <- c(
  "Cell.Types", "Sample.ID", "SORT", "donor_id", "Braak",
...
A Seurat object is already provided so we read it in.
No QC is neccessary as it was already applied by the authors.
...
Since a clustering and normalisation pipeline was already run by the authors, we just create visualisations here.
...
rds <- file.path(cache_dir, "seurat_prep.rds")
if (!file.exists(rds)) {
