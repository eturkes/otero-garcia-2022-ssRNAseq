...
for (i in seq_along(seurat_list)) {
  ...
  corr <- duplicateCorrelation(logcounts(gse), design, block = gse$donor_id)
  fit <- lmFit(
  ...
  corr <- duplicateCorrelation(logcounts(deg), design, block = deg$donor_id)
  fit <- lmFit(
...
results <- rbind(
  gse_list[[1]], gse_list[[2]], gse_list[[3]], gse_list[[4]], gse_list[[5]]
)
gse_corrections <- data.frame(mean = results[ , 2], p = results[ , 6])
gse_corrections <- ihw(p ~ mean,  data = gse_corrections, alpha = 0.05)
gse_corrections <- adj_pvalues(gse_corrections)

results <- rbind(
  deg_list[[1]], deg_list[[2]], deg_list[[3]], deg_list[[4]], deg_list[[5]]
)
deg_corrections <- data.frame(mean = results[ , 2], p = results[ , 6])
